...
- src/Service/MarkdownService.php is the container-visible entry point and the only piece that touches a disk. `toHtml()` renders, caching one converter per flavour because assembling a converter assembles its whole extension set. Before a converter is built, `configureEnvironment()` is called on its environment: empty here, it is where a child registers its own renderers — `symfony-design-system` draws tables through `data_table()` that way, as a second service rather than a replacement, so that a mail keeps plain html. `parse()` and `read()` build a document from a string or a file. `readFrontMatter()` exists next to `read()` for one reason: a list of a hundred and fifty documents is drawn from thirteen fields apiece, and it streams the head of the file and closes the handle at the closing fence rather than reading a hundred and fifty whole procedures.
